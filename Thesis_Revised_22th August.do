use "C:\Users\16056\OneDrive\Desktop\STATA DATA ANALYSIS\8- IMDS-4.dta" 
...
sum  protectvoluntary1  coordinatedfs1 comminvestdfs1
...
********************************************************************************
********************************************************************************
              /* Related variable want include in Model*/
********************************************************************************
********************************************************************************
...
replace ozdeadtrees1 = 80 if oz_9 == 4    
...
replace unhealthy = 0 if unhealthy != 1
replace time = 0 if time != 1 
replace zoning = 0 if zoning != 1 

replace fsi = 0 if fsi != 1 
replace money = 0 if money != 1
replace time2 = 0 if time2 != 1
replace hh_work = 0 if hh_work != 1 
replace hh_spend = 0 if hh_spend != 1 
replace iz_work = 0 if iz_work != 1 
replace iz_spend = 0 if iz_spend != 1 
replace oz_work = 0 if oz_work != 1 
replace oz_spend = 0 if oz_spend != 1 
...
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy

sum fire_risk if e(sample)
sum efficacy if e(sample) 
sum own_yrs if e(sample)

sum agehome if e(sample)
sum house_attr if e(sample)
sum land_attr if e(sample)
sum private if e(sample)
sum lot if e(sample)

sum risk_tolerance if e(sample)
sum income if e(sample)
sum own_more if e(sample)

sum aspect if e(sample)
sum elevdiff if e(sample)
sum slope if e(sample)
sum fuel if e(sample)
sum brush if e(sample)
sum grs if e(sample)
sum pj if e(sample)
sum tmbr if e(sample)
sum lghtn if e(sample)
sum wind if e(sample)

sum res if e(sample)
sum rx if e(sample)
sum road_more24 if e(sample)

sum zoning if e(sample)
sum time if e(sample)
sum health if e(sample)
sum age if e(sample)
...
drop if missing(defsp)
drop if missing(fire_risk)
drop if missing(efficacy)
drop if missing(hhvl)
drop if missing(own_yrs)
drop if missing(agehome)
drop if missing(house_attr)
drop if missing(land_attr)
drop if missing(private)
drop if missing(lot)
drop if missing(risk_tolerance)
drop if missing(income)
drop if missing(own_more)
drop if missing(age)
drop if missing(brush)
drop if missing(grs)
drop if missing(pj)
drop if missing(road_more24)
drop if missing(res)
drop if missing(rx)
drop if missing(fuel)
drop if missing(aspect)
drop if missing(slope)
drop if missing(elevdiff)
drop if missing(lghtn)
drop if missing(wind)
drop if missing(zoning)
drop if missing(time)
drop if missing(unhealthy)

*Table 1 - Summary Statistics: Nevada Communities
tab community
...
*Benefits of Fire-Safe Investment
sum fire_risk
sum efficacy
gen hhvniaz = exp(hhvl)
sum hhv
sum own_yrs
*Costs of Fire-Safe Investment
sum agehome
sum house_attr
sum land_attr
sum private
...
sum lghtn
sum wind
*Wildland Policy Variables
sum res
sum rx
sum road_more24
*Constraints on Fire-Safe Investment
sum zoning
sum time
sum health
sum age
**************End Drop and summary Stat************************************
***********************************************************************
*****************************************************************************
**************End Drop and summary Stat************************************
***********************************************************************
...
use "C:\Users\mhtaylor\Desktop\AS Thesis__Stata__24Sept2018\testdataset_thesis.dta" 
run ijwf_prep.do
*run ijwf_dropmissing.do


*Defensible Space
*(1)
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age 
mfx
*(2)
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx
mfx
*(3)
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind
mfx
*(4)
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj zoning time unhealthy
mfx
*(5)
probit defsp fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
...
probit fsi fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum fsi 
sum fsi if defsp == 1
probit money fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum money 
sum money if defsp == 1
probit time2 fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum time2 
sum time2 if defsp == 1

probit hh_spend fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum hh_spend
sum hh_spend if defsp == 1
probit hh_work fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum hh_work
sum hh_work if defsp == 1
probit iz_spend fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum iz_spend
sum iz_spend if defsp == 1
probit iz_work fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum iz_work
sum iz_work if defsp == 1
probit oz_spend fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum oz_spend
sum oz_spend if defsp == 1
probit oz_work fire_risk efficacy hhvl own_yrs agehome house_attr land_attr private lot risk_tolerance income own_more age brush grs pj road_more24 res rx fuel aspect slope elevdiff lghtn wind zoning time unhealthy
mfx
sum oz_work 
sum oz_work if defsp == 1
